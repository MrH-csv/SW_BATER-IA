...
#ifndef INA226_V1_COMPATIBILITY_MODE_
...
void INA226_V1::setConversionTime(INA226_CONV_TIME shuntConvTime, INA226_CONV_TIME busConvTime){
    uint16_t currentConfReg = readRegister(INA226_CONF_REG);
...
void INA226_V1::setConversionTime(INA226_CONV_TIME convTime){
    setConversionTime(convTime, convTime);
}

void INA226_V1::setMeasureMode(INA226_MEASURE_MODE mode){
...
void INA226_V1::setCurrentRange(INA226_CURRENT_RANGE range){ // deprecated, left for downward compatibility
    deviceCurrentRange = range;      
}

//set resistor and current range independant. resistor value in ohm, current range in A
void INA226_V1::setResistorRange(float resistor, float current_range){
...
void INA226_V1::startSingleMeasurementNoWait(){
    uint16_t val = readRegister(INA226_MASK_EN_REG); // clears CNVR (Conversion Ready) Flag
    val = readRegister(INA226_CONF_REG);
    writeRegister(INA226_CONF_REG, val);        // Starts conversion
}

void INA226_V1::powerDown(){
...
void INA226_V1::setAlertPinActiveHigh(){
    uint16_t val = readRegister(INA226_MASK_EN_REG);
    val |= 0x0002;
    writeRegister(INA226_MASK_EN_REG, val);
}

void INA226_V1::enableAlertLatch(){
    uint16_t val = readRegister(INA226_MASK_EN_REG);
    val |= 0x0001;
    writeRegister(INA226_MASK_EN_REG, val);
}

void INA226_V1::enableConvReadyAlert(){
    uint16_t val = readRegister(INA226_MASK_EN_REG);
    val |= 0x0400;
    writeRegister(INA226_MASK_EN_REG, val);
}
    
void INA226_V1::setAlertType(INA226_ALERT_TYPE type, float limit){
...
void INA226_V1::readAndClearFlags(){
    uint16_t value = readRegister(INA226_MASK_EN_REG);
...
uint8_t INA226_V1::getI2cErrorCode(){
    return i2cErrorCode;
}
...
void INA226_V1::writeRegister(uint8_t reg, uint16_t val){
  _wire->beginTransmission(i2cAddress);
...
uint16_t INA226_V1::readRegister(uint8_t reg){
